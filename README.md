...
**AI-assisted.** The implementation in `src/` and `tests/` was written with the
help of an AI coding assistant (Claude), working from my direction and reviewed
by me. Several report figures were generated by that code.

**My own work.** The choice of project and dataset; the decision to evaluate with
a track-disjoint split and the reasoning behind it; which ablations to run; every
training and evaluation run reported here; the interpretation of those results;
and the written report.

If your course requires a different form of declaration, follow that instead —
this statement is here because disclosure is better volunteered than discovered.
